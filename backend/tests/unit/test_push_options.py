...
def test_access_panel_completes_security_push_registration(app_client) -> None:  # type: ignore[no-untyped-def]
    query = "SN=ACC001&options=all&pushver=3.1.2&DeviceType=acc"
    # Security PUSH deliberately begins with OK, then requires /registry.
    initial = app_client.get(f"/iclock/cdata?{query}")
    ...
    configured = app_client.post("/iclock/push?SN=ACC001")
...
def test_post_with_options_all_is_not_discarded_as_handshake(app_client) -> None:  # type: ignore[no-untyped-def]
