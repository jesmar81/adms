...
export default function EnrollmentsPage() {
  ...
  return (
    <>
      ...
      {loading ? <LoadingState rows={5} /> : <DataTable ariaLabel="Solicitudes de enrolamiento" columns={requestColumns} data={requests} keyOf={(row) => row.id} renderCard={(row) => <div><p className="font-medium text-foreground">{employmentById.get(row.employment_id)?.employee_number ?? "Empleo"}</p><p className="mt-1 text-xs text-muted">{row.methods.join(", ")} · {row.status.replaceAll("_", " ")}</p>{row.fingerprint_positions.length ? <p className="mt-1 text-xs text-muted">{row.fingerprint_positions.map((finger) => FINGER_LABELS[finger] ?? finger).join(" · ")}</p> : null}</div>} empty={<EmptyState icon={<Fingerprint className="h-5 w-5" />} title="Sin solicitudes" description="Crea una solicitud y realiza el enrolamiento presencial en el equipo autorizado." />} />}
