...
import { CalendarDays, Clock3, Download, FileBarChart, LogIn, Pencil, Save, UserX } from "lucide-react";
...
export default function ReportsPage() {
  ...
  return <><PageHeader title="Reportes" description="Asistencia, tarjetas semanales y ajustes auditables de RR. HH." crumbs={[{ label: "Reportes" }]} />{error ? <div className="mb-5"><ErrorState error={error} onRetry={() => void runReport()} /></div> : null}<div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{REPORTS.map((report) => { const Icon = report.icon; return <button key={report.id} onClick={() => setKind(report.id)} className={`rounded-2xl border p-4 text-left ${kind === report.id ? "border-accent bg-accent-50" : "border-line-subtle bg-white hover:bg-zinc-50"}`}><Icon className="h-5 w-5 text-accent-700" /><p className="mt-3 font-semibold">{report.label}</p><p className="mt-1 text-sm text-zinc-500">{report.description}</p></button>; })}</div>{controls()}<div className="mt-5">{result()}</div>{editing ? <AdjustmentForm draft={editing} update={updateDraft} onCancel={() => setEditing(null)} onSave={() => void saveAdjustment()} /> : null}</>;
}
