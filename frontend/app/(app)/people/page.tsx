    ...
    <>
      ...
      <Card className="mb-5 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
          ...
          <div className="grid flex-1 gap-2 sm:grid-cols-2 xl:grid-cols-4"><Field label="Nombre">{(id) => <Input id={id} value={firstName} onChange={(e) => setFirstName(e.target.value)} disabled={!groupId} />}</Field><Field label="Apellido paterno">{(id) => <Input id={id} value={lastName} onChange={(e) => setLastName(e.target.value)} disabled={!groupId} />}</Field><Field label="Apellido materno">{(id) => <Input id={id} value={secondLastName} onChange={(e) => setSecondLastName(e.target.value)} disabled={!groupId} />}</Field><Field label="Correo">{(id) => <Input id={id} type="email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={!groupId} />}</Field></div>
          <Button variant="primary" icon={<Plus className="h-4 w-4" />} onClick={() => void create()} loading={saving} disabled={!groupId || !firstName.trim() || !lastName.trim()}>Crear persona</Button>
        </div>
      </Card>
