...
export default function DashboardPage() {
  ...
  return <>
    <section className="mb-7 flex flex-col justify-between gap-5 border-b border-line-subtle pb-6 lg:flex-row lg:items-end"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-medium text-emerald-300"><span className="relative flex h-1.5 w-1.5"><span className="absolute h-full w-full animate-pulse-dot rounded-full bg-emerald-400" /><span className="relative h-1.5 w-1.5 rounded-full bg-emerald-400" /></span>Centro de control operativo</div><h1 className="text-2xl font-semibold tracking-tight text-foreground md:text-[30px]">{greeting()}{user ? `, ${user.username}` : ""}</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">Indicadores calculados contra horarios, feriados, ajustes y solicitudes reales; no son datos simulados.</p></div>{companies.length > 0 && <div className="flex flex-wrap items-end gap-2 rounded-lg border border-line-subtle bg-surface-raised p-2"><label className="grid gap-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-muted">Empresa<select value={companyId} onChange={(event) => setCompanyId(event.target.value)} className="h-8 min-w-44 rounded-md border border-line-soft bg-surface px-2 text-xs font-medium text-foreground outline-none transition focus:border-accent">{companies.map((company) => <option key={company.id} value={company.id}>{company.trade_name || company.legal_name}</option>)}</select></label><label className="grid gap-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-muted">Día operativo<input type="date" value={reportDate} onChange={(event) => setReportDate(event.target.value)} className="h-8 rounded-md border border-line-soft bg-surface px-2 text-xs text-foreground outline-none transition focus:border-accent" /></label></div>}</section>
